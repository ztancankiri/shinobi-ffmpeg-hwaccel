...
RUN \
    echo "**** install Intel Driver & Tools ****" && \
    apt-get update -y && \
    apt-get install -y \
...
RUN git clone --branch master https://gitlab.com/Shinobi-Systems/Shinobi.git . && \
    npm install && \
    npm install pm2 -g

RUN curl https://gitlab.com/Shinobi-Systems/ShinobiDocker/-/raw/master/entrypoint.sh?ref_type=heads -o entrypoint.sh && \
...
ENTRYPOINT ["/home/Shinobi/entrypoint.sh"]